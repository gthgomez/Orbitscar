...
export const COLONY_SCHEMA_VERSION = 1;
...
export function parseColonySave(serialized: string): ColonyState { let parsed: unknown; try { parsed = JSON.parse(serialized); } catch { throw new Error("colony save is not valid JSON"); } if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) throw new Error("colony save must be an object"); const save = parsed as Partial<ColonySave>; if (save.schemaVersion !== COLONY_SCHEMA_VERSION || save.payload === undefined || typeof save.checksum !== "string") throw new Error("unsupported colony save schema"); if (authoritativeDigest(save.payload) !== save.checksum) throw new Error("colony save checksum mismatch"); return clone(save.payload); }
