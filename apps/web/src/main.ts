...
class BreachScene extends Phaser.Scene {
  ...
  update(_time: number, delta: number): void { if (!this.result) return; this.elapsed = Math.min(this.result.durationTicks / 30 * 1000, this.elapsed + delta); this.simTicks = Math.floor(this.elapsed / 1000 * 30); while (this.eventIndex < this.result.events.length && this.result.events[this.eventIndex].tick <= this.simTicks) this.applyEvent(this.result.events[this.eventIndex++]); this.draw(); }
  ...
  private draw(): void {
    ...
    const fps = this.game.loop.actualFps || 0; const entityCount = this.units.size + structures.length; const status = this.result ? `${this.result.winner.toUpperCase()} / ${this.result.victoryTier.toUpperCase()}` : "READY"; this.hud.setText(`ORBITSCAR // BREACH LAB\n${status}   t=${this.simTicks}/${this.result?.durationTicks ?? 0}   reserves ${this.result?.survivingUnits ? Object.values(this.result.survivingUnits).reduce((a, b) => a + b, 0) : 0}\nFPS ${fps.toFixed(0)}  entities ${entityCount}  events ${this.result?.eventCount ?? 0}  zoom ${this.cameras.main.zoom.toFixed(2)}`); for (const [zone, text] of this.zoneTexts) text.setStyle({ color: zone === this.zone ? "#8ee6d1" : "#b8c8cf" });
  }
