...
import { attackAgain, beginDeployment, beginArmyComposition, canStageWave, clearAttackPlan, countStaged, createGameSession, MAX_DEPLOYMENT_CHARGES, restartPlan, showReport, startBattle, type GameSession, type Zone } from "./state/game-session.js";
...
function createBattleInput(): OrbitscarBattleInput {
  const commands: OrbitscarBattleInput["commands"] = session.plan.waves.map((wave, index) => ({ commandId: `wave-${index + 1}`, sequence: index + 1, tick: index * 600, type: "DEPLOY" as const, payload: { zone: wave.zone, position: zonePosition(wave.zone), units: wave.units } }));
  if (session.plan.abilityArmed) commands.push({ commandId: "commander-reroute", sequence: commands.length + 1, tick: 300, type: "COMMANDER_ABILITY", payload: { abilityId: "emergency_reroute", targetStructureId: "arc" } });
...
function zonePosition(zone: Zone): { x: number; y: number } { return { ...({ west: { x: 120, y: 400 }, north: { x: 420, y: 90 }, south: { x: 420, y: 710 }, east: { x: 1080, y: 400 } } as const)[zone] }; }
